...
# Fallback static headlines (used if all RSS feeds fail)
FALLBACK_NEWS = {
    "ai":     [{"title":"OpenAI releases GPT-5 with advanced reasoning capabilities","source":"TechCrunch","time":"2h ago","category":"AI"},
               ...
               {"title":"Anthropic Claude 4 scores highest on MMLU benchmark","source":"Bloomberg","time":"6h ago","category":"AI Business"},
    ...
    "crypto": [{"title":"Bitcoin hits new all-time high above $100K","source":"CoinDesk","time":"1h ago","category":"Markets"},
...
def news_agent(msg):
    ...
    # Filter by keyword if specific topic asked (e.g. "iran news")
    topic_words = [w for w in m.split() if len(w) > 3 and w not in
                   {"news","latest","today","show","tell","give","what","about","articles","headlines","recent","update"}]
    if topic_words and articles:
        filtered = [a for a in articles if any(tw in a["title"].lower() for tw in topic_words)]
        if filtered:
            articles = filtered

    # Fallback to static if RSS failed
    if not articles:
...
CRITICAL RULES:
- When tool_data is provided in [AGENT DATA], use it as the primary source of truth. Present it clearly.
- For weather: state the city, temperature, condition, humidity, wind, UV index clearly.
- For crypto: show price, 24h change %, market cap.
- For news: present each headline naturally with source.
- For database: confirm actions clearly and show results.
- For calculator/converter: state the result directly.
- For ANY other question (history, science, coding, general knowledge, explanations, opinions, etc.) — answer fully from your own knowledge. You are a capable general-purpose assistant, not just an agent router.
- NEVER say you cannot access real-time data when tool_data is provided.
- NEVER refuse to answer general knowledge questions. Answer them directly and helpfully.
- Be concise, confident, and helpful. No AI disclaimers. Format with clear line breaks.
- If asked about yourself: you are AI Nexus, powered by LLaMA 3.3 70B via Groq.
"""
...
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=JetBrains+Mono:wght@300;400;500;600;700;800&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
/* Force Bebas Neue with multiple fallbacks */
...
:root{
  ...
  --purple:#9d7cff; --pdim:rgba(157,124,255,0.1);
...
body::before{
  ...
  background:
    radial-gradient(ellipse 80% 50% at 20% -10%,rgba(200,255,71,0.07) 0%,transparent 60%),
    radial-gradient(ellipse 60% 40% at 80% 110%,rgba(61,184,255,0.06) 0%,transparent 60%),
...
.glass::after{
  ...
  background:linear-gradient(135deg,rgba(255,255,255,0.05) 0%,rgba(255,255,255,0) 50%,rgba(0,0,0,0.06) 100%);
}
...
.sb::before{content:'';position:absolute;top:-80px;left:-60px;width:280px;height:280px;background:radial-gradient(circle,rgba(200,255,71,0.1) 0%,transparent 65%);pointer-events:none;z-index:0}
.sb>*{position:relative;z-index:1}

.sb-top{padding:20px 16px 16px;border-bottom:1px solid rgba(255,255,255,0.05)}
.sb-live{display:inline-flex;align-items:center;gap:5px;background:rgba(200,255,71,0.1);border:1px solid rgba(200,255,71,0.25);border-radius:20px;padding:3px 9px;margin-bottom:12px}
...
.sb-logo{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:40px;color:#fff;line-height:.95;letter-spacing:.03em}
.sb-logo span{color:var(--a)}
.sb-sub{font-size:9px;color:var(--t3);letter-spacing:.1em;margin-top:6px;font-family:var(--mono)}
...
.ni::before{content:'';position:absolute;left:0;top:20%;bottom:20%;width:2px;background:var(--a);transform:scaleY(0);transform-origin:center;border-radius:2px;transition:transform .2s var(--ease)}
...
.ni.active{
  ...
  border-color:rgba(200,255,71,0.2);color:var(--a);
  box-shadow:0 0 20px rgba(200,255,71,0.06);
}
.ni.active::before{transform:scaleY(1)}
.ni-ic{width:28px;height:28px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:13px;background:rgba(255,255,255,0.04);flex-shrink:0}
.ni.active .ni-ic{background:rgba(200,255,71,0.1)}
.ni-name{flex:1;font-weight:600}
...
.nt-new{background:rgba(157,124,255,0.12);color:var(--purple)}
...
.sb-stat-n{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:32px;color:#fff;line-height:1}
...
.tb-ic{
  ...
  background:rgba(200,255,71,0.1);border:1px solid rgba(200,255,71,0.22);
  transition:all .25s var(--ease);
  box-shadow:0 0 16px rgba(200,255,71,0.08);
}
.tb-title{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:22px;color:#fff;letter-spacing:.05em;line-height:1;transition:all .2s}
.tb-sub{font-size:8px;color:var(--a);letter-spacing:.1em;margin-top:2px;opacity:.7;font-family:var(--mono);transition:all .2s}
...
.qbar{
  ...
  background:rgba(4,6,15,0.4);backdrop-filter:blur(20px);
...
.e-orb{
  ...
  margin:0 auto 18px;
  box-shadow:0 0 60px rgba(200,255,71,0.1),0 0 120px rgba(200,255,71,0.04);
  position:relative;
}
.e-orb::before{content:'';position:absolute;inset:-10px;border-radius:28px;border:1px solid rgba(200,255,71,0.06)}
.e-title{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:64px;color:#fff;letter-spacing:.05em;line-height:.9;margin-bottom:4px}
...
.e-card:hover{border-color:rgba(200,255,71,0.2);background:rgba(200,255,71,0.06);transform:translateY(-3px);box-shadow:0 12px 32px rgba(0,0,0,0.3)}
...
.e-chip{
  ...
  background:rgba(255,255,255,0.03);font-size:10.5px;color:var(--t1);cursor:pointer;
...
.bub-u{
  ...
  border:1px solid rgba(200,255,71,0.1);
...
.thinking{
  ...
  background:rgba(12,16,32,0.5);border:1px solid rgba(255,255,255,0.07);
...
.card-h-t{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:15px;color:var(--t1);letter-spacing:.1em;flex:1}
...
.w-temp{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:72px;color:#fff;line-height:1;text-shadow:0 0 40px rgba(200,255,71,0.25)}
.w-unit{font-size:24px;color:var(--t3)}
.w-city{font-size:13px;font-weight:700;color:var(--t1);margin-top:5px;font-family:var(--body)}
.w-cond{font-size:9.5px;color:var(--t2);margin-top:3px;font-family:var(--mono)}
.w-emoji{font-size:52px;filter:drop-shadow(0 0 20px rgba(255,220,100,0.3))}
...
.n-num{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:26px;color:rgba(255,255,255,0.12);min-width:28px;line-height:1;margin-top:1px}
...
.c-price{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:26px;color:#fff;line-height:1}
...
.aok{
  ...
  background:rgba(45,255,176,0.05);border:1px solid rgba(45,255,176,0.15);
...
.res-val{font-family:'Bebas Neue','Impact','Arial Narrow',sans-serif;font-size:60px;color:var(--a);text-shadow:0 0 30px rgba(200,255,71,0.35);line-height:1}
...
.inp-wrap{
  ...
  border:1px solid rgba(255,255,255,0.09);
  border-radius:16px;padding:4px 4px 4px 18px;
...
.inp-wrap:focus-within{border-color:rgba(200,255,71,0.28);box-shadow:0 0 0 3px rgba(200,255,71,0.06),0 8px 32px rgba(0,0,0,0.3)}
.inp-wrap textarea{
...
.send{
  ...
  box-shadow:0 4px 16px rgba(200,255,71,0.3);
}
.send:hover{background:#d4ff5a;box-shadow:0 6px 24px rgba(200,255,71,0.4);transform:scale(1.07)}
...
.inp-albl{margin-left:auto;color:var(--a);opacity:.6;font-size:8.5px;transition:all .2s}

/* FLASH */
@keyframes agentFlash{0%{opacity:0}40%{opacity:.05}100%{opacity:0}}
...
<script>
// Verify Bebas Neue font is loaded
document.fonts.ready.then(()=>{
  const loaded=[...document.fonts].map(f=>f.family);
  if(!loaded.some(f=>f.includes('Bebas'))){
    // Font didn't load - inject fallback style
    const s=document.createElement('style');
    s.textContent=`
      .sb-logo,.e-title,.tb-title,.card-h-t,.w-temp,.c-price,.res-val,.n-num,.sb-stat-n{
        font-family:'Impact','Arial Narrow',Arial,sans-serif !important;
        letter-spacing:.04em !important;
      }
    `;
    document.head.appendChild(s);
    console.warn('Bebas Neue failed to load, using Impact fallback');
  }
});
</script>
...
const META={
  ...
  news:{tag:'📰 NEWS',bg:'rgba(157,124,255,.12)',fg:'#9d7cff'},
...
function renderNews(d){
  const liveTag = d.live ? `<span style="font-size:7px;padding:2px 6px;border-radius:3px;background:rgba(45,255,176,0.12);color:var(--green);font-weight:700;letter-spacing:.08em;font-family:var(--mono)">● LIVE RSS</span>` : `<span style="font-size:7px;padding:2px 6px;border-radius:3px;background:rgba(255,168,48,0.12);color:var(--amber);font-weight:700;letter-spacing:.08em;font-family:var(--mono)">CACHED</span>`;
  ...
  return `<div class="card"><div class="card-h"><span class="card-h-ic">📰</span><span class="card-h-t">NEWS FEED · ${esc(d.category).toUpperCase()}</span><div style="display:flex;align-items:center;gap:6px">${liveTag}<span class="card-h-s">updated ${d.last_updated}</span></div></div><div class="card-b">${items}</div></div>`;
}
