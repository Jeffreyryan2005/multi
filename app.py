...
<link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;1,400&family=Syne:wght@700;800&display=swap" rel="stylesheet">
<style>
...
.sb-logo{font-family:var(--syne);font-size:21px;font-weight:800;color:#fff;letter-spacing:-.02em;line-height:1}
...
.sb-stat-n{font-size:22px;font-weight:800;color:#fff;line-height:1;font-family:var(--syne)}
...
.e-title{font-family:var(--syne);font-size:28px;font-weight:800;color:#fff;letter-spacing:-.03em;margin-bottom:5px}
...
/* ── CARDS ── */
.card{background:var(--panel);border:1px solid var(--b1);border-radius:10px;overflow:hidden;margin-top:8px;animation:msgIn .35s var(--ease) both;box-shadow:0 4px 24px rgba(0,0,0,0.25)}
.card-h{padding:8px 14px;border-bottom:1px solid var(--b1);display:flex;align-items:center;gap:8px;background:var(--p2)}
.card-h-ic{font-size:12px}
.card-h-t{font-size:9px;font-weight:700;color:var(--t1);letter-spacing:.07em;flex:1}
...
.w-temp{font-family:var(--syne);font-size:46px;font-weight:800;color:#fff;line-height:1;text-shadow:0 0 30px rgba(198,255,78,0.18)}
...
.n-num{font-family:var(--syne);font-size:18px;font-weight:800;color:var(--p3);min-width:22px;line-height:1;margin-top:2px}
...
.c-price{font-family:var(--syne);font-size:17px;font-weight:800;color:#fff;line-height:1}
...
.res-val{font-family:var(--syne);font-size:36px;font-weight:800;color:var(--a);text-shadow:0 0 20px var(--ag)}
...
async function send(){
  ...
  br.innerHTML=`<div class="av av-b">⚡</div><div class="msg-body" id="bb"><div class="msg-meta" id="bm"></div>
    <div class="thinking" id="thinker"><div class="td"></div><div class="td"></div><div class="td"></div></div>
    <div class="bub bub-b" id="bbub" style="display:none"></div><div id="bcard"></div></div>`;
  ...
  try{
    ...
    document.getElementById('bm').innerHTML=`<span class="atag" style="background:${m.bg};color:${m.fg}">${m.tag}</span> ${getTime()}`;
    const bub=document.getElementById('bbub'); bub.style.display='inline-block';
    typeEffect(data.response,bub,()=>{
      const card=buildCard(agent,data.tool_data);
      if(card) document.getElementById('bcard').innerHTML=card;
